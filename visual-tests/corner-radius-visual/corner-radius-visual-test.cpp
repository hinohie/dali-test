...
const std::string EXPECTED_IMAGE_FILE =
    TEST_IMAGE_DIR "corner-radius-visual/expected-result.png";
...
public:
  ...
  void OnInit(Application &application) {
    ...
    mTerminateTimer.TickSignal().Connect(
        this, &CornerRadiusVisualTest::OnTerminateTimer);
  ...
  void CreateVisuals(bool isAnimation, bool isRelative) {
    ...
    if (isAnimation) {
      ...
      mAnimation.FinishedSignal().Connect(
          this, &CornerRadiusVisualTest::OnFinishedAnimation);
      mAnimation.Play();
    }
  }
  ...
  void CreateTestSet(std::uint32_t visualTestTypeIndex,
    ...
    for (std::uint32_t subSizeTestTypeIndex = 0; subSizeTestTypeIndex < 4;
      ...
      control.ResourceReadySignal().Connect(this,
                                            &CornerRadiusVisualTest::OnReady);
            ...
            if (pair.first.indexKey == DevelVisual::Property::CORNER_RADIUS) {
              if (pair.second.GetType() == Property::FLOAT) {
                ...
                mAnimation.AnimateTo(DevelControl::GetVisualProperty(
                                         control, Control::Property::BACKGROUND,
                                         pair.first.indexKey),
                                     convertedValue);
              } else {
                mAnimation.AnimateTo(DevelControl::GetVisualProperty(
  ...
  Property::Map CreateTestVisualMap(std::uint32_t propertyTestTypeIndex,
    ...
    if (isRelative) {
      ...
      if (value.GetType() == Property::FLOAT) {
        visualMap[DevelVisual::Property::CORNER_RADIUS] =
