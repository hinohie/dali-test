...
const ModelDetails MODELS[] = {
    {"AntiqueCamera.usdz", Vector3(0.0f, -1.2f, 0.0f), 0.0035f,
     Quaternion(Radian(Degree(15.0f)), Vector3::YAXIS)},
    {"BarramundiFish.usdz", Vector3(0.0f, -1.0f, 0.0f), 0.08f,
     Quaternion(Radian(Degree(-90.0f)), Vector3::YAXIS)},
    {"CesiumMilkTruck.usdz", Vector3(0.0f, -0.5f, 0.0f), 0.005f,
     Quaternion(Radian(Degree(-40.0f)), Vector3::YAXIS)},
    {"Corset.usdz", Vector3(0.0f, -0.7f, 0.0f), 0.3f,
     Quaternion(Radian(Degree(30.0f)), Vector3::XAXIS)},
    {"WaterBottle.usdz", Vector3(0.0f, 0.0f, 0.0f), 0.08f,
     Quaternion(Radian(Degree(30.0f)), Vector3::XAXIS)},
};
...
private:
  void PrepareNextTest() {
    ...
    mScene.SetProperty(Dali::Actor::Property::ORIENTATION,
                       nextModel.orientation);
  ...
  void PostRender(std::string outputFile, bool success) {
    CompareImageFile(TEST_IMAGE_DIR "usd-model/expected-result-" +
                         std::to_string(gTestStep + 1) + ".png",
                     outputFile, 0.98f);
...
DALI_VISUAL_TEST_WITH_WINDOW_SIZE(UsdModelTest, OnInit, WINDOW_WIDTH,
                                  WINDOW_HEIGHT)
