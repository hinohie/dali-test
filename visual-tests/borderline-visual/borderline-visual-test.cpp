...
const std::string EXPECTED_IMAGE_FILE =
    TEST_IMAGE_DIR "borderline-visual/expected-result.png";
...
public:
  BorderlineVisualTest(Application &application) : mApplication(application) {}
  ...
  void OnInit(Application &application) {
    ...
    mTerminateTimer.TickSignal().Connect(
        this, &BorderlineVisualTest::OnTerminateTimer);
  ...
  void CreateVisuals(bool isAnimation, bool requiredPreMulitpliedAlpha) {
    ...
    if (isAnimation) {
      ...
      mAnimation.FinishedSignal().Connect(
          this, &BorderlineVisualTest::OnFinishedAnimation);
      mAnimation.Play();
    }
  }
    ...
    for (std::uint32_t subSizeTestTypeIndex = 0; subSizeTestTypeIndex < 4;
      ...
      if (isAnimation) {
        ...
        if (testVisualMap.Find(DevelVisual::Property::CORNER_RADIUS_POLICY)) {
          basicVisualMap.Insert(
              DevelVisual::Property::CORNER_RADIUS_POLICY,
              testVisualMap[DevelVisual::Property::CORNER_RADIUS_POLICY]);
        }
        ...
        for (std::uint32_t mapIndex = 0; mapIndex < testVisualMap.Count();
          ...
          if (pair.first.type == Property::Key::Type::INDEX) {
            ...
            if ((pair.first.indexKey ==
                 DevelVisual::Property::BORDERLINE_WIDTH) ||
                (pair.first.indexKey ==
                 DevelVisual::Property::BORDERLINE_COLOR) ||
                (pair.first.indexKey ==
                 DevelVisual::Property::BORDERLINE_OFFSET) ||
                (pair.first.indexKey == DevelVisual::Property::CORNER_RADIUS)) {
              mAnimation.AnimateTo(DevelControl::GetVisualProperty(
                                       imageView, ImageView::Property::IMAGE,
                                       pair.first.indexKey),
                                   pair.second);
            }
          }
      ...
      // For clean up
      mControlList.emplace_back(imageView);
    }
  }
...
private:
  ...
  Property::Map CreateTestVisualMap(std::uint32_t propertyTestTypeIndex,
    ...
    if (propertyTestTypeIndex >= (NUMBER_OF_PROPERTY_TYPES / 2)) {
      ...
      visualMap[DevelVisual::Property::CORNER_RADIUS_POLICY] =
...
DALI_VISUAL_TEST_WITH_WINDOW_SIZE(
    BorderlineVisualTest, OnInit, TESTSET_VISUAL_SIZE *NUMBER_OF_PROPERTY_TYPES,
